...
| 35 | `extensao_desempate_busca_produto_fuzzy_12-09.sql` | TRB-2026-0004: `buscar_produto_fuzzy` tinha `order by similaridade desc` sem desempate — em empate de similaridade o Postgres não garante ordem estável entre chamadas, causa estrutural confirmada por leitura da definição (bate com o incidente real de troca silenciosa de produto). Acrescenta `p.nome asc, p.id asc` como desempate, sem mudar `WHERE`/match. **PENDENTE** — sessão sem `execute_sql`/`apply_migration` (ver nota de execução no arquivo); não deu pra reproduzir empiricamente a instabilidade nem aplicar/validar ao vivo. |
| 36 | `extensao_seguranca_p5_revoga_anon_tabelas_legado_12-09.sql` | TRB-2026-0010: fecha o "Achado 3" do item 34 — revoga INSERT/UPDATE/DELETE de `anon` em 31 tabelas antigas do dashboard (`clientes`, `mensagens`, `operadores`, `produtos`, etc.) que tinham o grant completo desde o baseline de 26/08 e nunca foram revogadas. RLS + policies gateadas por `eh_operador_ativo()`/`eh_admin()` já bloqueavam a escrita (confirmado por leitura de `pg_policies` do baseline + `rls_enabled=true` ao vivo via `list_tables`); nenhum fluxo legítimo (bot = `service_role`, dashboard = sessão `authenticated`) depende do grant. `authenticated`/`service_role` intactos. **PENDENTE** — sessão sem `execute_sql`/`apply_migration`; não deu pra confirmar ao vivo os grants atuais de `anon` nem aplicar/validar (ver nota de execução no arquivo). |
