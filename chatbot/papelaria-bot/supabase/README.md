...
| 49 | `extensao_segunda_rodada_pesquisa_22-09.sql` | Segunda rodada da mesma tarefa (22/09/2026): pesquisa externa dos 78 itens que ficaram com cadastro leve no item 47 (`pendente_pesquisa`) — borrachas/apontadores, canetas/canetinhas/marca-texto, giz de cera, lápis de cor, lapiseiras/grafites — mais a revalidação de Strong e "Mais+" do item 48. UPDATE, não INSERT (produtos já existiam). Achados registrados com fonte e confiança em cada descrição; 13 itens ficaram com `precisa_validacao` explícito e 1 correção de entendimento (SKU 39121 não é lápis de cor, é grafite comum com cabo neon — mesma categoria "Lápis", que já mistura os dois tipos). Aplicado e validado ao vivo: 78/78 produtos atualizados, 0 com `pendente_pesquisa` restante. |
| 50 | `extensao_resolucao_validacoes_22-09.sql` | Mesmo dia, pesquisa adicional mais direcionada (código de barras, catálogo de distribuidor, comparação de preço por candidato) pra tentar resolver os 13 itens `precisa_validacao` do item 49. Resultado: **8 resolvidos** com evidência concreta — destaque pros 2 cadernos: "Strong" era na verdade a linha oficial Tilibra **"Hide"** vendida sob apelido de gôndola (nome de fabricante corrigido só na descrição, `nome`/SKU do produto continuam fiéis ao ticket ShopControl); "Mais+" confirmado como linha real vendida em 2 apresentações (brochura 48fls e espiral 96fls — o cadastro já estava certo). **5 continuam `precisa_validacao`** genuinamente (apontador FC "Since 1761", caneta Trilux "vermelha/azul", grafite FC 0.5 "2B/HB", lapiseira Sky Paper "SK1303", canetinha Sky Paper Jumbo 24 cores) — todos com limite real de pesquisa por internet, resolvem só comparando com a embalagem física na loja. Aplicado e validado ao vivo. Detalhe completo no Documento 08 da base de conhecimento. |
