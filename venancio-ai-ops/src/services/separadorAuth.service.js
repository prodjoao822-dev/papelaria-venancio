...
// direto do navegador: é o bot quem decide se o PIN bate e quem controla
// o bloqueio por tentativas. O bot devolve um access_token/refresh_token
// de uma sessão Supabase Auth já válida, que aplicamos no client ISOLADO
// (separadorSupabase, ver separadorClient.js) via setSession().
...
export const separadorAuthService = {
  async login(codigoFuncionario, pin) {
    if (!BOT_API_URL) {
      throw new Error('VITE_BOT_API_URL não configurado — login indisponível.')
    }
